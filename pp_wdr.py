...
os.chdir(r"D:\AE EWEM MSc\T H E S I S\6_code\code repository\run_data\wdr")
analysed_flowdata_paths = ['dtu/flowdata.nc', 'wdr/flowdata.nc']
...
# Set to True to post process velocity deficits (takes a while)
pp_advd_option = False
...
if gh_option:
    ...
    analysed_downstream_xs = np.arange(
        first_probed_distance, last_probed_distance, analysed_spacing)
...
# 2: d\overline(u'v')/dy
# 3: d\overline(u'w')/dw
# range(4) for mtt terms, range(6) to plot u'v' and u'w' too
mtt_option = False
mtt_cmap = cm.RdBu

vel_option = False
vel_cmap = cm.jet
vel_option_hori_long = False
...
def calc_UAD(data, x, y, z, D, n=128, var='U'):
    ...
    for i in range(len(x)):
        ...
        A = (y_rec[1] - y_rec[0]) * (z_rec[1] - z_rec[0])
...
def pp_advd(filename_path):
    ...
    ax.plot(x / D, ADvar / UH, label=labelname)
    return labelname, probed_advd
...
def pp_gh(filename_path, n=128, color='r'):
    ...
    # Vertical line of the velocity profile
    z_line = 1.5 * np.linspace(-R, R, n) + zh

    data = xarray.open_dataset(filename_path)

    for i, pos in enumerate(analysed_downstream_xs):
        print(f"x/D = {pos}")
        # Velocity deficit [-] (0 none, 1 fully stopped flow)
        U_profiles[i] = analysed_spacing * \
            (data.U.interp(x=pos*D, y=0, z=z_line) / UH)
        U_hat = U_profiles[i]
        ...
        ax.plot(U_hat + pos - analysed_spacing, (z_line - (29.85 + D/2)
                                                 ) / D, color=color)  # 29.85m  is the distance to ground
        ax.axvline(pos, color='k', linestyle='dotted')
        text_x_offset = -1e-1 * analysed_spacing
        text_y_offset = -2.5e-1 * analysed_spacing
        ax.text(pos + text_x_offset, text_y_offset,
                f"x/D = {pos}", rotation=90)
...
def pp_tke(sigma_data, axis_counter):
    ...
    tim_z = np.sqrt(zplane_data['tke'].T * (2/3)) / \
        UH  # Turbulence intensity measure
    left_plot = axes[axis_counter, 0].contourf(X / D, Y / D, tim_z,
                                               cmap=tke_cmap,
                                               levels=np.linspace(0.059, 0.17, 11))
...
def pp_vel_hori_long(sigma_data, axis_counter):
    ''' Post-process turbulent kinetic energy.
    '''
    current_label = r"$\overline{\delta}$ = " + \
        str(analysed_deltas[axis_counter])
    offset = 0
    ...
    zplane_data = sigma_data.interp(z=zh)  # Slice at hub height
    X, Y = np.meshgrid(zplane_data.x, zplane_data.y)
    vel_z = zplane_data['U'].T/UH  # Streamwise velocity
    left_plot = axes[axis_counter].contourf(X / D + offset, Y / D, vel_z,
                                            cmap=vel_cmap,
                                            levels=np.linspace(0.3, 1.1, 9))
    axes[axis_counter].text(x_limit[0] + 1e-1,
                            y_limit[1] + 0.5e-1,
                            current_label)

    plt.xlim(x_limit)
    axes[axis_counter].set_ylim(y_limit)
...
def pp_mtt(sigma_data, mtt_term_index, axis_counter):
    ...
    max_x = np.float(sigma_data['x'][-1])
    max_y = np.float(sigma_data['y'][-1])
    max_z = np.float(sigma_data['z'][-1])

    x_limits = np.array([-2, 12]) * D
    y_limits = np.array([-1.5, 1.5]) * D
    z_limits = np.array([0, max_z])

    x_space = np.linspace(*x_limits)
    y_space = np.linspace(*y_limits)
    z_space = np.linspace(*z_limits)
    #x_space = [-max_x, max_x]
    #y_space = [-max_y, max_y]
    #z_space = [0, max_z]
    ...
    # Normalize by -D/k_ref
    k_ref = sigma_data['tke'].interp(
        x=-1.5*D, y=0, z=zh)  # reference TKE [m^2/s^2]
    mtt_normaliser = -D/k_ref  # [m s^2]
    zplane_data['term1'] = zplane_data['term1'] * mtt_normaliser
    zplane_data['term2'] = zplane_data['term2'] * mtt_normaliser
    zplane_data['term3'] = zplane_data['term3'] * mtt_normaliser
    yplane_data['term1'] = yplane_data['term1'] * mtt_normaliser
    yplane_data['term2'] = yplane_data['term2'] * mtt_normaliser
    yplane_data['term3'] = yplane_data['term3'] * mtt_normaliser
    #print(f"k_ref ={k_ref}")

    # Integrate(ish)
    t2 = zplane_data['term2'].interp(x=x_space, y=y_space)
    t2_pos = np.sum(t2.where(t2 > 0, 0).integrate("y"))
    print(f"t2_pos = {np.float(t2_pos)}")
    t2_neg = np.sum(t2.where(t2 < 0, 0).integrate("y"))
    print(f"t2_pos = {np.float(t2_neg)}")
    M_lateral = np.sum(t2_pos - t2_neg)
    #M_lateral_1 = np.sum(zplane_data['term2'].interp(x=x_space, y=y_space_1).integrate("y"))
    #M_lateral_2 = np.sum(zplane_data['term2'].interp(x=x_space, y=y_space_2).integrate("y"))
    #M_lateral = M_lateral_1 + M_lateral_2
    print(f"M_lateral = {np.float(M_lateral)}")
    t3 = yplane_data['term3'].interp(x=x_space, z=z_space)
    t3_pos = t3.where(t3 > 0, 0).integrate("z")
    t3_neg = t3.where(t3 < 0, 0).integrate("z")
    M_vertical = np.sum(np.sum(t3_pos) - np.sum(t3_neg))
    #M_vertical_b = np.sum(yplane_data['term3'].interp(x=x_space, z=z_space_1).integrate("z"))
    #M_vertical_t = np.sum(yplane_data['term3'].interp(x=x_space, z=z_space_2).integrate("z"))
    #M_vertical = M_vertical_b + M_vertical_t
    #print(f"M_vertical_t>M_vertical_b: {bool(M_vertical_t>M_vertical_b)}")
    print(f"M_vertical = {np.float(M_vertical)}")
    ...
    if mtt_term_index == 0:
        ...
        current_xlabel = r"    $-\dfrac{D}{k_{ref}}\dfrac{\partial \overline{u' u'}}{\partial x} \ [-]$"
        plot_data_l = zplane_data['term1'].T
        plot_data_r = yplane_data['term1'].T
        current_levels_l = np.linspace(-10, 10, 10)  # -0.02 -> 0.02
        current_levels_r = np.linspace(-10, 10, 10)  # -0.02 -> 0.02
    elif mtt_term_index == 2:
        current_xlabel = r"$\overline{u' w'}$"  # wrong
        plot_data_l = zplane_data['term2'].T
        plot_data_r = yplane_data['term3'].T
        current_levels_l = np.linspace(-32, 32, 10)  # -0.36 -> 0.12
        current_levels_r = np.linspace(-32, 32, 10)  # -1.2 -> 1.2
    # elif mtt_term_index == 2:
    #     current_xlabel = r"$\dfrac{\partial \overline{u' v'}}{\partial y} \ [-]$"
    #     plot_data_l = zplane_data['term2'].T
    #     plot_data_r = yplane_data['term2'].T
    #     current_levels_l = np.linspace(-32, 32, 10) # -0.06 -> 0.06
    #     current_levels_r = np.linspace(-32, 32, 10) # -0.016 -> 0.048
    ...
    return left_plot, right_plot, current_xlabel, M_lateral, M_vertical
...
if __name__ == "__main__":
    ...
    if pp_adbladeloads_option:
        ...
        if check_against_DES:
            fig.savefig('wdr_blade_loads_val.pdf', bbox_inches='tight')
        else:
            fig.savefig('wdr_blade_loads.pdf', bbox_inches='tight')
    ...
    if pp_advd_option:
        ...
        fig.savefig('wdr_vel_deficits.pdf', bbox_inches='tight')
    ...
    if gh_option:
        ...
        for gh_iterator, filename_path in enumerate(analysed_flowdata_paths):
            ...
            print(
                f"Drawing gaussian hats for delta={analysed_deltas[gh_iterator]}...")
            print(color)
            pp_gh(filename_path, color=color)

        # Cut the graph to show the wake beyond the wind turbine width
        wake_margin = 0.
        # (29.85)/D + 0.5 = 0.6674, height of hub [D]
        plt.ylim([-0.6674 - wake_margin, 0.6674 + wake_margin])

        # The following block makes the 1|0, 0.5, 0|1... ticks on the x axis
        n_ticks = len(analysed_downstream_xs)/2 + \
            2  # Number of ticks we will have
        x_ticks = np.arange(first_probed_distance,
                            last_probed_distance, analysed_spacing / 2)[:-1]
        internal_ticks = int(n_ticks) * ["1|0", "0.5"]
        x_labels = [*internal_ticks, "1"]
        latex_fontsize = 18
        plt.xticks(ticks=x_ticks, labels=x_labels)
        plt.ylabel(r"$\tilde{z}/D \ [-]$", fontsize=latex_fontsize)
        plt.xlabel("$U/U_{H} \ [-]$", fontsize=latex_fontsize)

        # Now let us create a hardcoded legend for this graph
        hardcode_legend_elements = [mpl.lines.Line2D([0], [0], color=colors[0], lw=2, label=l1),
                                    mpl.lines.Line2D([0], [0], color=colors[1], lw=2, label=l2)]
        ax.legend(handles=hardcode_legend_elements, loc="upper left")

        ax.set_xlim([-analysed_spacing*1.3, (n_hats-2)*analysed_spacing])
        fig.savefig('../../plots/wdr_jou_gaussian_hats.pdf',
                    bbox_inches='tight')
    ...
    if tke_option:
        ...
        for tke_iterator, filename_path in enumerate(analysed_flowdata_paths):
            print(
                f"Ploting turbulence intensity measure {tke_iterator + 1}/{n_deltas}")
        ...
        colorbar_left.ax.set_xlabel("    $I$ $[-]$")
        colorbar_right.ax.set_xlabel("    $I$ $[-]$")

        axes[n_deltas-1, 0].set_xlabel("$x/D$ $[-]$")
        axes[n_deltas-1, 1].set_xlabel("$x/D$ $[-]$")

        axes[1, 0].set_ylabel("$y/D$ $[-]$")
        axes[1, 1].set_ylabel(r"\tilde{z}/D $[-]$")
        fig.savefig('../../plots/wdr_jou_tke.pdf', bbox_inches='tight')

    # %% Plot RANS equation terms

    M_matrix = np.zeros((2, 3))
    M_counter = 0
    ...
    if mtt_option:
        for mtt_index in mtt_option:
            ...
            for mtt_iterator, filename_path in enumerate(analysed_flowdata_paths):
                print(
                    f"Ploting momentum transport term {mtt_index}:  {mtt_iterator + 1}/{n_deltas}")

                data = xarray.open_dataset(filename_path)
                pped_data = calc_sigma(data)

                left_plot, right_plot, current_xlabel, M_lateral, M_vertical = pp_mtt(
                    pped_data, mtt_index, mtt_iterator)
                ...
                if mtt_index == 1:
                    print(f"mtt_index = {mtt_index}")
                    M_matrix[M_counter, :] = current_delta, M_lateral, M_vertical
                    M_counter += 1
            ...
            if mtt_index == 1:
                ...
                colorbar_left.ax.set_xlabel(
                    r"    $-\dfrac{D}{k_{ref}}\dfrac{\partial \overline{u' v'}}{\partial y} \ [-]$")
                colorbar_right.ax.set_xlabel(
                    r"    $-\dfrac{D}{k_{ref}}\dfrac{\partial \overline{u' w'}}{\partial z} \ [-]$")
            ...
            fig.savefig(
                f'../../plots/wdr_jou_mtt_{mtt_index}.pdf', bbox_inches='tight')

            print(f"M_matrix:\n{M_matrix}")
            delta_M = np.copy(M_matrix)
            delta_M[1, 1:3] = M_matrix[1, 1:3] / M_matrix[0, 1:3]
            #delta_M[2, 1:3] = M_matrix [2, 1:3] / M_matrix [1, 1:3]
            print(f"D_matrix:\n{delta_M}")
    ...
    if vel_option:
        ...
        axes[1, 0].set_ylabel("$y/D$ $[-]$")
        axes[1, 1].set_ylabel(r"$\tilde{z}/D$ $[-]$")
        fig.savefig('../../plots/wdr_jou_vel.pdf', bbox_inches='tight')

    if vel_option_hori_long:
        ...
        for vel_iterator, filename_path in enumerate(analysed_flowdata_paths):
            ...
            left_plot = pp_vel_hori_long(pped_data, vel_iterator)

            current_delta = analysed_deltas[vel_iterator]

        colorbar_left = fig.colorbar(left_plot, ax=axes[:])
        ...
        fig.savefig('../../plots/wdr_jou_vel_hori_long.pdf',
                    bbox_inches='tight')
