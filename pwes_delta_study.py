...
\overline{\delta} parameter in the Joukowsky `2xxx` forcing model of 
PyWakeEllipSys.
"""
# Imports
import numpy as np
...
def run_delta_study(ad_jou_rootdelta=0.25,
                    ad_jou_roota=2.335,
                    ad_jou_rootb=4.0,
                    # Grid resolution [l, m, f, uf], see config.py
                    grid=config.f_res_grid,
    ...
    e3d_model = EllipSys(wf, wt, wf.Ti, wt.zRef,  # Required positional arguments
                         ...
                         casename='dtu10mw',
                         # Inner margin, west, after rotation [D]
                         grid_m1_w_D=3.0,
                         # Inner margin, east, after rotation [D]
                         grid_m1_e_D=12.0,
                         # Inner margin, south, after rotation [D]
                         grid_m1_s_D=1.5,
                         # Inner margin, north, after rotation [D]
                         grid_m1_n_D=1.5,
    ...
    if run:
        ...
        for delta in delta_variants:
            ...
            xyz = [20 * wt.D, 2 * wt.D, wt.zRef + wt.D]
