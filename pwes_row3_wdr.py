"""
This code was made for the purposes of studying the wake performance 
of the re-designed wake diffusion rotor configuration of the DTU10MW reference
wind turbine blade in PyWakeEllipSys.
"""
...
class DTU10MW(EllipSysOneTypeWT):
    def __init__(self):
        EllipSysOneTypeWT.__init__(self,
                                   ...
                                   bladegeo_file=f'{design_name}_blade.geo',
...
def run_pwes_simulation(
        ...
        ad_jou_roota=2.335,
        ad_jou_rootb=4.0,
        grid=config.l_res_grid,  # Grid resolution [l, m, f], see config.py
    ...
    print(f"3 Row total P = {np.sum(power_ilk)} MW.")

    fname = f"{design_name}_row3_power.csv"
    np.savetxt(fname, power_ilk, delimiter="\t")

# =============================================================================
#     new_power = float(power_ilk)/1e6 # MW
...
# =============================================================================
    ...
    xyz_3 = [15 * wt.D, 2 * wt.D, wt.zRef + wt.D]
