...
def hawc2s_files_to_geo(design_name, save=True):
    ...
    if save:
        np.savetxt(f"{design_name}_blade.geo", geo_mat,
                   comments=comments, header=header)  # comments=str(N)
...
def run_hawc2s(design_name):
    ...
    if 'my_dtu_10mw' not in cwd:
        # HAWC2S must be ran from the folder with the .htc file
    ...
    stream = stream.read()
    print(
        f"CMD OUTPUT FOR THE '{design_name}' DESIGN\n-------------------------------------\n{stream}")
...
def pp_hawc2s_ind(design_name, U=8, rho=1.225):
    ...
    print(f"{'{design_name} C_T: ':<20}" +
          f"{str(np.round(new_design_C_T, 4))} [-].")
...
def pp_hawc2s_bladepower(design_name):
    ...
    s_nd = s / S           # Blade span, non-dimensioned [-]
    C_P = data[:, 33]  # Power coefficient [-]
    powers = C_P[1:] * 0.5 * rho * np.pi * \
        (s[1:]**2 - s[:-1]**2) * U**3 / 1e6  # [MW]

    total_power = np.sum(powers)
    percentage_of_reported = total_power / new_power * 100
    print(f"{'{design_name} summed P: ':<20}" +
          f"{str(np.round(total_power, 4))} MW.")
    print(f"{'Percentage of P: ':<19}" +
          f"{str(np.round(percentage_of_reported, 2))} %.")
